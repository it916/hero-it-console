...
// desde varios módulos (tickets, solicitudes, reportes, conexiones),
...
export default {
  async fetch(request, env) {
    ...
    // para poder probar el Hub en desarrollo contra este Worker.
    ...
    // (Firebase ID token), así que va antes del gate del Console.
    //
    // NO hay lista blanca: cualquier persona con
    ...
    if (request.method === 'GET' && path === '/stats') {
      try {
        ...
        const [tickets, solicitudes] = await Promise.all([
...
// margen. Evita firmar JWT RS256 + round-trip a Google en cada request
